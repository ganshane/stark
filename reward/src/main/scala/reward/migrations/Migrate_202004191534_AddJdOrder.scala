...
class Migrate_202004191534_AddJdOrder extends Migration{
  ...
  override def up(): Unit = {
    createTable("jd_order_info"){t=>
      ...
      t.column("union_traffic_group",VarcharType,Limit(100))
      t.column("valid_code",VarcharType,Limit(100))
      t.column("sub_union_id",VarcharType,Limit(100))
      t.column("trace_type",VarcharType,Limit(100))
      ...
      t.column("union_role",VarcharType,Limit(100))
      t.column("gift_coupon_key",VarcharType,Limit(100))
      t.column("gift_coupon_ocs_amount",IntegerType)
    }
    addIndex("jd_order_info",Array("order_id","sku_id"),Unique)
  }
